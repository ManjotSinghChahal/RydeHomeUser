...
class SharedPrefModule(private val context: Context) {
    ...
    companion object {
        ...
        private var instance: SharedPrefModule? = null

        fun init(context: Context) {
            if (instance == null) {
                instance = SharedPrefModule(context)
            }
        }

        fun getInstance(): SharedPrefModule? {
            if (instance == null) {
             //   instance = App.instance?.let { SharedPrefModule(it) }
            }
            return instance
        }
...
//
    ...
    fun clear() {
        ...
        mSharedPreferences?.let {
            //  it.edit().clear().apply()
            mEditor = mSharedPreferences?.edit()
            SharedPrefModule.getInstance()?.saveEmail("")
            SharedPrefModule.getInstance()?.saveAuthToken("")
            SharedPrefModule.getInstance()?.saveUserId("")
            mEditor!!.apply()
